   ...
   @include('applicant.sidebar')
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
      <div class="main-content">
        ...
        <form method="POST" action="{{ route('siblings.store') }}">
          @csrf
              ...
              <tbody>
                @if(isset($siblings) && $siblings->count() > 0)
                    @foreach($siblings as $index => $sibling)
                        <tr data-id="{{ $sibling->id }}">
                            <td data-label="No">{{ $index + 1 }}</td>
                            <td data-label="Name"><input type="text" class="form-control" name="name[]" value="{{ $sibling->siblingName }}" required></td>
                            <td data-label="Age"><input type="number" class="form-control" name="age[]" value="{{ $sibling->siblingAge }}" required></td>
                            <td data-label="Date of Birth"><input type="date" class="form-control" name="dob[]" value="{{ $sibling->siblingDOB }}" required></td>
                            <td data-label="Occupation"><input type="text" class="form-control" name="occupation[]" value="{{ $sibling->occupation }}" required></td>
                            <td data-label="Employer/Institution"><input type="text" class="form-control" name="emp_institude[]" value="{{ $sibling->emp_ins }}" required></td>
                            <td data-label="Action">
                                <button type="button" class="btn btn-danger btn-sm remove-row" data-id="{{ $sibling->id }}">Remove</button>
                            </td>
                        </tr>
                    @endforeach
                @else
    ...
    $(document).on('click', '.remove-row', function() {
        ...
        if (id) {
            ...
            if (confirm('Are you sure you want to delete this sibling?')) {
                $.ajax({
                    url: "{{ url('/siblings') }}/" + id,  // Update this line
                    type: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
